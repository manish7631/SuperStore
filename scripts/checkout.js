    ...
    for(var i=0;i<arr.length;i++){
       ...
       if(flag==1){
            addressStore.push(list);
             localStorage.setItem("Super_addressList",JSON.stringify(addressStore));
...
var summary=JSON.parse(localStorage.getItem("Super_cart"));
displayItem(summary);
function displayItem(summary){
      summary.map(function(item){
            ...
            div1.textContent=  "Name:" + item.name;
            var div2=document.createElement("div");
            div2.setAttribute("id","div2");
            
            var p2=document.createElement("p");
            p2.setAttribute("class","p2");
            p2.textContent= "Price" + " " +"Rs."+item.price;
            div2.append(p2);
...
var total = cartitems.reduce(function (acc, cv) {
  ...
  document.querySelector("#total").innerHTML = `<h1>Total cost is ${ "Rs." + total} `
